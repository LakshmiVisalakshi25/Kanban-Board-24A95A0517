...
export default function TaskModal({ isNew = true, task, columnId, data, setData, onClose }) {
  const columns = [
    { id: "todo", title: "To Do" },
  ...
  const assignees = ["Lakshmi", "Team"]; // Manager removed
  ...
  function handleSubmit(e) {
    ...
    if (!task) {
      ...
      updatedColumns[columnId] = updatedColumns[columnId].filter((t) => t.id !== task.id);
      updatedColumns[column] = [...updatedColumns[column], newTask];
    }

    const newData = { ...data, columns: updatedColumns };
    setData(newData);
    onClose();

    const status = document.getElementById("aria-status");
    if (status) status.innerText = task ? "Task updated" : "Task added";
  }

  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
      <form className="bg-white p-6 rounded shadow-lg w-96" onSubmit={handleSubmit}>
        <h2 className="text-lg font-bold mb-4">{isNew ? "Add Task" : "Edit Task"}</h2>
